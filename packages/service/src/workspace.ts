...
export function renderAgentsMd(config: IcarusConfig): string {
  ...
  // Инструкция по Maple подключается только если сервер реально настроен:
  // иначе агент будет обещать то, чего у него нет.
...
- Код — на языке Maple: «;» печатает результат, «:» подавляет; diff, int, dsolve,
  assume, evalf, Pi, infinity. Сомневаешься в синтаксисе — сначала
  mcp_maple_check_code.
- Один вызов живёт около 25 секунд (клиентский таймаут): длинный расчёт дроби
  на шаги, иначе вызов оборвётся.
